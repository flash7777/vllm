...
class Attention(nn.Module, AttentionLayerBase):
    ...
    def __init__(
        ...
        **extra_impl_args,
        ...
        # MultiQuant: init buffers for compressed KV-cache (TQ/RQ tq3/tq4/...)
        # Coexists with upstream's _init_turboquant_buffers (which gates on
        # 'turboquant_*' dtypes). MultiQuant gates on 'tq[234][wr]?' / 'rq[234]'.
        ...
        # _tq_enabled is the legacy multiquant alias; upstream uses _tq_config
        # only when its own gating fires (turboquant_* dtypes), so no clash.
    ...
    def _init_multiquant_buffers(
        ...
        MultiQuant has its own quantizer registry separate from upstream's
        TurboQuantConfig — gates on tq[234][wr]?/rq[234] dtypes. Buffers are
        registered under _tq_* names; upstream's _init_turboquant_buffers
        only fires for turboquant_* dtypes, so the namespace is shared but
        the gating is disjoint.
        """
        import re
    ...
    def get_kv_cache_spec(self, vllm_config: VllmConfig) -> KVCacheSpec | None:
        ...
        if self.sliding_window is not None:
            ...
            # MultiQuant TQ/RQ: use packed cache_head_size from mq_config so
            # FlashInfer/Triton attention sees the compressed slot shape.
