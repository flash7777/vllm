...
// XFP MoE GEMM v14 — SMEM A-row cache (v12) + bulk cp.async B preload.
//
// Companion to xfp_gemm_v14.cu. For MoE policy K_SMEM_MAX=4096 so the
// 16-group B cap (V14_N_GROUPS_MAX=16 in core.cuh) is not a binding
// constraint on Qwen/GLM shapes (K=2048 → n_groups=8 at bits=4).
    ...
    int64_t num_valid_tokens)
{
    ...
    TORCH_CHECK(K % 2 == 0,
                "xfp_moe_gemm v14: K must be even, got K=", K);
    TORCH_CHECK(K <= MoEPolicy::K_SMEM_MAX,
                "xfp_moe_gemm v14: K=", K, " exceeds s_A K_SMEM_MAX=",
    ...
    TORCH_CHECK(n_groups <= 16,
                "xfp_moe_gemm v14: n_groups=", n_groups,
    ...
    if (bits == 2)      launch_moe<2>(A, B_packed, codebook, C, sorted_token_ids, expert_ids, topk_weights,
                                      ...
                                      static_cast<int>(top_k), static_cast<int>(flat_per_expert),
                                      static_cast<int>(num_valid_tokens));
    else TORCH_CHECK(false, "xfp_moe_gemm v14: unsupported bits=", bits);
}
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    m.def("xfp_moe_gemm", &multiquant::xfp_moe_gemm,
          "XFP MoE v14: SMEM A-row cache + bulk cp.async B preload");
}
