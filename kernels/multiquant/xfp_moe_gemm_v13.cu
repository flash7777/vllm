...
// XFP MoE GEMM v13 — v12 SMEM A-row cache + v11_async cp.async B prefetch.
//
// Combines both core orthogonal optimizations for the MoE policy: A-row is
// loaded once into __shared__ s_A[MoEPolicy::K_SMEM_MAX=4096] and shared
// across the block's 8 warps, while B_packed is double-buffer prefetched
// via cp.async so the next group's global-to-shared move hides behind the
// current group's slot compute.
//
// Bitwise identical to MoE v11/v12. K ≤ 4096, covers all Qwen/GLM MoE
// shapes (K=2048) with 2× headroom.
    ...
    int64_t num_valid_tokens)
{
    ...
    TORCH_CHECK(K % 2 == 0,
                "xfp_moe_gemm v13: K must be even, got K=", K);
    TORCH_CHECK(K <= MoEPolicy::K_SMEM_MAX,
                "xfp_moe_gemm v13: K=", K, " exceeds K_SMEM_MAX=",
                MoEPolicy::K_SMEM_MAX, " — use v11 for this shape");
    ...
    if (bits == 2)      launch_moe<2>(A, B_packed, codebook, C, sorted_token_ids, expert_ids, topk_weights,
                                      ...
                                      static_cast<int>(top_k), static_cast<int>(flat_per_expert),
                                      static_cast<int>(num_valid_tokens));
    else TORCH_CHECK(false, "xfp_moe_gemm v13: unsupported bits=", bits);
}
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    m.def("xfp_moe_gemm", &multiquant::xfp_moe_gemm,
          "XFP MoE v13: SMEM A-row cache + cp.async B prefetch");
}
